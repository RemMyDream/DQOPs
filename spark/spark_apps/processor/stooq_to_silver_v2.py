...
logger = create_logger("stooq_to_silver")
...
class SilverIngestionService:    
    ...
    def process(self, tickers: List[str]) -> int:
        self._logger.info(f"Processing tickers: {tickers}")
        
        bronze_df = self.spark.table(f"{self.source_catalog}.stooq")
        
        processed_dfs = []
        for ticker in tickers:
            self._logger.info(f"Processing {ticker}...")
        ...
        if not table_exists:
            self._logger.info(f"Creating new table: {silver_table}")
            final_df.write.format("iceberg").mode("overwrite").saveAsTable(silver_table)
        else:
            self._logger.info(f"Merging into: {silver_table}")
            self._merge_into_table(final_df, silver_table, ["Date", "ticker"])
        
        row_count = self.spark.table(silver_table).count()
        self._logger.info(f"{silver_table}: {row_count} total rows")
        return row_count

def process_stooq_to_silver(
    spark: SparkSession,
    tickers: List[str] = ["NVDA","MSFT"],
    source_catalog: str = "bronze",
    target_catalog: str = "silver"
) -> int:
    service = SilverIngestionService(spark, source_catalog, target_catalog)
    return service.process(tickers)
